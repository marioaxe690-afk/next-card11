...
export async function POST(request: Request) {
  ...
  try {
    const result = await backendPorts.calendar.createOrUpdate(body);
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { providerId: `ics:${body.targetId}`, status: "failed", error: error instanceof Error ? error.message : "Calendar failed" },
      { status: 500 }
    );
  }
}
